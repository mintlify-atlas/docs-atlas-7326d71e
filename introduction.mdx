---
title: "Introduction"
description: "tally is a BuildKit-native Dockerfile and Containerfile linter and formatter with 105+ rules, safe auto-fix, LSP support, and SARIF output."
---
...
<CardGroup cols={2}>
  ...
  <Card title="Rules reference" icon="list-check" href="/rules/overview">
    105+ rules across BuildKit, tally, Hadolint, and ShellCheck namespaces.
  </Card>
...
### 105+ rules across three namespaces

| Namespace | Rules | Source |
|---|---|---|
| `buildkit/` | 22 | Docker's official Dockerfile checks |
| `tally/` | 46 | Custom rules including secret detection with gitleaks |
| `hadolint/` | 37 | Hadolint-compatible Dockerfile rules |

Shell scripts inside `RUN` instructions are additionally analyzed via embedded ShellCheck (rules under the `shellcheck/` namespace — coverage is expanding).
